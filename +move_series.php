...
<body>
  ...
  <?php
    if (isset($_POST['submit'])) {
      ...
      if (empty($_POST['title'])) {
        message("the title is empty","red");
        $require = 0;
      }
      if (empty($_POST['description'])) {
        message("the description is empty","red");
        $require = 0;
      }
      if (empty($_POST['date'])) {
        message("the date is empty","red");
        $require = 0;
      }
      if (empty($_FILES['poster']['name'])) {
        message("the poster is empty","red");
        $require = 0;
      }
      if (empty($_FILES['trailer']['name'])) {
        message("the trailer is empty","red");
        $require = 0;
      }
      if ($require) {
        ...
        if (in_array($poster_ext, $photo_ext) && in_array($trailer_ext, $video_ext)) {
          ...
          $new = array('name' => $name, 'description' => $_POST['description'], 'date' => $_POST['date']);
          ...
          if(isset($_POST['type'])){
            $ms=$_POST['type'];
            $sql = "UPDATE movie SET movie=$ms WHERE id=$id";
  ...
  <form class="form" action="" method="POST" enctype="multipart/form-data">
    ...
    <div class="input-container">
      <input name="description" class="input" type="text" placeholder=" " />
      <div style="width: 90px" class="cut"></div>
      <label for="description" class="placeholder">description</label>
    </div>
    
    <div class="input-container">
      <input name="date" class="input" type="date" placeholder=" " />
    ...
    <div class="file">
      ...
      <input class="file__input" type="file" name="poster" value="" />
    </div>
    
    <div class="file">
      <span class="material-symbols-outlined"> upload </span>
      <label class="file__lable" for="trailer">Add trailer</label>
      <input class="file__input" type="file" name="trailer" value="" />
    </div>
    ...
    <div class="radio">
      <input class="radio__input" type="radio" id="movie" name="type" value="1" checked />
      <label class="radio__lable" for="movie" >movie</label>
      <input class="radio__input" type="radio" id="series" name="type" value="0"  />
